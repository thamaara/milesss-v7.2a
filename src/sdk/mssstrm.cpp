    ...
    if (s->preload) {
      ...
      if (bn) {
        ...
        if (s->reset_ASI[s->buf1])
        {
          ...
          AIL_load_sample_buffer(s->samp,MSS_BUFFER_HEAD,s->bufs[s->buf1],s->size1);
        }
        else
        {
          AIL_load_sample_buffer(s->samp,MSS_BUFFER_HEAD,s->bufs[s->buf1],s->size1);
...
static S32 setflags(HSTREAM stream, U32 type, U8 *subtype, U32 bits, U32 chans, U32 mask)
{
  type = type;
   ...
   SS_set_sample_type(stream->samp,stream->filetype,stream->fileflags,mask);
...
domp3:
      ...
      if ( LE_SWAP32( &tag )==0x46464952) 
         ...
         if ( LE_SWAP32( &tag )==0x45564157) 
            ...
            if (type == WAVE_FORMAT_EXTENSIBLE)       // Multichannel PCM .WAV
               {
...
static S32 doread(HSTREAM stream,S32 cur, S32* size)
{
  ...
  if (amt)
  {
    ...
    if ( align_to_next_2k )
    {
      if ( ( align_to_next_2k < amt ) && ( (amt-align_to_next_2k) > 2048 ) && ( ( align_to_next_2k & ( AIL_API_sample_granularity( stream->samp ) - 1 ) ) == 0 ) )
      {
...
static S32 check_buffers( HSTREAM stream, S32 fillup )
{
  ...
  MSSLockedIncrementPtr(stream->noback);
   ...
   AIL_lock();
                AIL_API_sample_buffer_info(stream->samp, 0, &pos0, &len0, NULL, NULL);
   S32 result = AIL_API_sample_buffer_info(stream->samp, 1, &pos1, &len1, NULL, NULL);
   AIL_unlock();
...
S32  AILCALL AIL_API_stream_position(HSTREAM stream)
{
  ...
  if (stream) {
    if (stream->error==0) {
      ...
      AIL_lock();
      AIL_sample_buffer_info(stream->samp,0,&pos0,&len0, NULL, NULL);
      AIL_sample_buffer_info(stream->samp,1,&pos1,&len1, NULL, NULL);
      AIL_unlock();
