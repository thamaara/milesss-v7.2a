                                                  ...
                                                  S32       ICA_enable)
{
         ...
         case CALLBACK_PFX:
            ...
            if (S->prefix_callback != NULL)
               ...
               MSS_do_cb3_with_ret( S->shadow.indirect[log], (AILPREFIXCB),
                  S->prefix_callback,S->driver->callingDS,
                  S, log, data_2);
               }
            break;
         ...
         case CLEAR_BEAT_BAR:
            ...
            if (S->beat_callback != NULL)
               ...
               MSS_do_cb4( (AILBEATCB),
                 S->beat_callback, S->driver->callingDS,
                   mdi, S, 0, 0);
               }
         ...
         case CALLBACK_TRIG:
            ...
            if (S->trigger_callback != NULL)
               ...
               MSS_do_cb3( (AILTRIGGERCB),
                 S->trigger_callback, S->driver->callingDS,
                 S, log, data_2);
               }
   ...
   if (mdi->event_trap != NULL)
      {
      MSS_do_cb5_with_ret( result, (AILEVENTCB), mdi->event_trap, mdi->callingDS,
...
static void AILLIBCALLBACK XMI_serve(HMDIDRIVER mdi) 
{
   //
                  ...
                  case EV_META:
                     ...
                     switch (type)
                        ...
                        case META_TIME_SIG:
                           ...
                           q = 16000000 / AIL_preference[MDI_SERVICE_RATE];
...
static void init_mdi_defaults(HMDIDRIVER mdi)
{
   S32 i;
...
void AILCALL AIL_API_close_XMIDI_driver( HMDIDRIVER mdi )
{
   HMDIDRIVER cur,prev;
   S32        i;
...
#define OPENERR 0
HMDIDRIVER AILCALL AIL_API_open_XMIDI_driver( U32 flags )
{
   S32 i;
   HMDIDRIVER mdi;
   ...
   AIL_memset( mdi, 0, sizeof( MDI_DRIVER ) );
   ...
   mdi->sequences = (HSEQUENCE) AIL_mem_alloc_lock(sizeof(struct _SEQUENCE) * mdi->n_sequences);
   ...
   AIL_memset( mdi->sequences, 0, sizeof( struct _SEQUENCE ) * mdi->n_sequences );
   ...
   mdi->interval_time = 1000000 / AIL_preference[MDI_SERVICE_RATE];
   ...
   AIL_set_timer_user(mdi->timer,(UINTa) mdi);
...
S32 AILCALL AIL_API_init_sequence(HSEQUENCE S, void const FAR *start, S32 sequence_num)
{
   ...
   len = 8 + BEU_SWAP32( ((U8 FAR *) (image+4)));
   end = image + len;
   ...
   while (image < end)
      {
      if (!AIL_strnicmp((char*)image,"TIMB",4))
         {
         S->TIMB = image;
         }

      if (!AIL_strnicmp((char*)image,"RBRN",4))
         {
         S->RBRN = image;
         }
   ...
   i = 8 + BEU_SWAP32( ((U8 FAR *) (((char FAR *) S->TIMB)+4)));
   if ( i > sizeof(TIMB) ) {
      i = sizeof(TIMB);
   }
   AIL_memcpy(TIMB, S->TIMB, i);

   T = (TIMB_chunk *)(void FAR*) TIMB;
   ...
   if (S->driver->timbre_trap != NULL)
      ...
      while (i < (U32) LE_SWAP16(&T->n_entries))
         {
         patch  =  ((U32) LE_SWAP16(&T->timbre[i])) & 0xff;
         bank   = (((U32) LE_SWAP16(&T->timbre[i]) & 0xff00)) >> 8;

         MSS_do_cb3_with_ret(result,(AILTIMBRECB),
               S->driver->timbre_trap,S->driver->callingDS,
         ...
         if (!result)
            ...
            for (j = i+1; j < (U32) LE_SWAP16(&T->n_entries); j++)
               {
               T->timbre[j-1] = T->timbre[j];
               }

            T->n_entries = (U16)( LE_SWAP16(&T->n_entries) - 1);
            MEM_LE_SWAP16(&T->n_entries);
...
void AILCALL AIL_API_end_sequence(HSEQUENCE S)
{
   ...
   if (S->EOS != NULL)
      {
      MSS_do_cb1( (AILSEQUENCECB),S->EOS,S->driver->callingDS,
        S);
      }
}
...
void AILCALL AIL_API_branch_index(HSEQUENCE S, U32 marker)
{
   ...
   for (i=0; i < n; i++)
      {
      if ((U32)LE_SWAP16(&R[i].bnum) == marker)
         {
         break;
         }
      }
   ...
   S->EVNT_ptr = (U8*)AIL_ptr_add((U8 FAR *) S->EVNT,8 + LE_SWAP32(&R[i].offset));
...
void AILCALL AIL_API_set_sequence_user_data(HSEQUENCE S, U32 index, SINTa value)
{
   if (S == NULL)
      {
      return;
...
SINTa AILCALL AIL_API_sequence_user_data(HSEQUENCE S, U32 index)
{
   if (S == NULL)
      {
      return 0;
                                            ...
                                            S32       milliseconds)
{
   ...
   CTRL_LOG original;
   AIL_memcpy(&original, &S->shadow, sizeof(original));
