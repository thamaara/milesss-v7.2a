...
static UINTa AIL_fopen(char const FAR* name, U32 acc)
{
   ...
   char buf[ 512 ];
   FNVALUES fnv;
   get_subfile_info( buf, name, &fnv );

   opdata.name = fnv.filename;
   ...
   if ( fnv.start >= opdata.size )
   {
     fnv.start = opdata.size;
     fnv.size = 0;
   }
   else if ( fnv.size > ( opdata.size - fnv.start ) )
   {
     fnv.size = ( opdata.size - fnv.start );
   }
   ...
   U32 fh = AIL_lowopen(opdata.file_handle, fnv.size, fnv.start );
...
static void read_callback( s32 result, DVDFileInfo* d )
{
  ...
  else
  {
    ...
    // OSReport("still outstanding: %d\n",outstanding->bytes_to_read );
  }
...
static void cancel_dvd( DVDFileInfo * file_info )
{
  U32 status;

  async_reading = 0;

  DVDCancel( (DVDCommandBlock *) file_info );

  // Wait until any outstanding reads are finished.
  do
  {
    status = DVDGetDriveStatus();
  } while ( ( status == DVD_STATE_WAITING ) || ( status == DVD_STATE_BUSY ) );
}
...
static void WiiHandleError( void )
{
  struct WII_IO_ASYNC_READ_DATA * cur;
  BOOL i;
  ...
  if ( cur )
    cancel_dvd( cur->file_info );

  while ( cur )
  {
    cur->status = WII_STATUS_CANCELLED;
    cur = cur->next;
  }
...
static U32 check_status( DVDFileInfo * file_info )
{
  U32 status;
...
static UINTa AIL_fopen(char const FAR* name, U32 acc)
{
  ...
  if ( DVDOpen( fnv.filename, &d ) )
  {
    //
    // Open succeeded -- wrap the descriptor in our PLATFILE structure
    //
    U32 size = DVDGetLength(&d);
    ...
    else if ( fnv.size > ( size - fnv.start ) )
    {
      fnv.size = ( size - fnv.start );
    }

    U32 fh = AIL_lowopen((S32)&d, fnv.size, fnv.start);
                                             ...
                                             U32  FAR *BytesRead)
{
  ...
  if (wait)
  {
    ...
    {
      if ( check_status( ma->stream_param.file_info ) )
        ma->stream_param.status = WII_STATUS_ERROR;
    }
  }
  else
  {
...
static U32 AIL_lowopen( SINTa frn, S32 len, S32 pos )
{
  ...
  if (frn!=BADFILEHANDLE)
  {
    ...
    pf->file_handle=(short)frn;

    if (pos==-1)
      {
      ::GetFPos(pf->file_handle,(long FAR*)&pf->start_pos);
      }
   else
      {
      pf->start_pos = pos;
    ...
    if (len==-1)
    {
      ::SetFPos(pf->file_handle,fsFromLEOF,0);
      ::GetFPos(pf->file_handle,(long FAR*)&pf->file_length);
      ::SetFPos(pf->file_handle,fsFromStart,pf->start_pos);
                                                ...
                                                U32 Bytes)
{
  ...
  for (i = 0 ; i < MAX_ASYNCS ; i++ )
  {
    if ( pf->asyncs[i].in_use == 0 )
    {
      ...
      pf->asyncs[i].in_use = 1;
      pf->asyncs[i].stream_param.ioRefNum = pf->file_handle;
...
typedef MSS_STRUCT PLATFILE       // Not Mac async (FSS), in general...
{
  UINTa file_handle;
  U32 file_length;
  U32 start_pos;
...
static UINTa AIL_lowopen( SINTa hand, S32 len, S32 pos )
{
  ...
  if (hand!=(SINTa)BADFILEHANDLE)
  {
    ...
    if (pos == -1)
      ...
      {
      pf->start_pos = pos;
      AIL_lowfseekbegin(hand,pf->start_pos);   // if explicit position passed in, make sure it agrees with the file system
      }
...
static UINTa AIL_fopen( char const FAR* name,U32 acc )
{
  ...
  hand=AIL_lowfopen( fnv.filename, acc );

  U32 size;

  size=AIL_lowfseekend(hand,0);
  ...
  fh=AIL_lowopen( hand, fnv.size, fnv.start );
...
static S32 AIL_fseekbegin(UINTa hand,S32 offset)
{
  ...
  return( AIL_lowfseekbegin(pf->file_handle,offset+pf->start_pos ) - pf->start_pos );
}
...
static S32 AIL_fseekcur(UINTa hand,S32 offset)
{
  ...
  U32 new_pos = (AIL_lowfseekcur(pf->file_handle,offset ) - pf->start_pos) ;

  if (new_pos > pf->file_length)
    return( AIL_lowfseekend(pf->file_handle,0) );
...
{
  U32 amt=0;
  AIL_doread(((PLATFILE FAR*)FileHandle)->file_handle,Buffer,Bytes,amt);
  return(amt);
}
