...
static S32 clear_dsbuf(void* dsbuf, S32 bits)
{
 ...
 again:
  ...
  {
    //
    // Restore, and try again if OK
    //
    // Return 0 if restore attempt failed (which should only happen when
...
S32 AILCALL get_system_speaker_configuration(MSS_MC_SPEC *channel_spec)
{
   ...
   *channel_spec = MSS_MC_STEREO;
   ...
   switch (DSSPEAKER_CONFIG(config & 0xff))    // Must mask off geometry byte before comparing
      {
...
MMRESULT DIG_waveOutReset(HDIGDRIVER dig, HWAVEOUT hWaveOut)
{
   ...
   S32 attempts = 0; // Limit retries to avoid device-removal lockups on Vista  
...
void AILCALL SS_start_DIG_driver_playback(HSAMPLE S)
{
   ...
   if (AIL_preference[DIG_USE_WAVEOUT])
      ...
      HRESULT result = DSERR_BUFFERLOST;

      if (clear_dsbuf(dig->DS_out_buff,dig->wformat.wBitsPerSample))
         {
         S32 attempts = 0;
                                ...
                                LPWAVEFORMAT     lpFormat)
{
   ...
   *drvr = dig;
   ...
   //
   // If a voice filter is in use (0 physical channels/sample), the default DMA rate
   // is whatever the user asked for.  dig->DMA_rate will be modified in the mixer/filter 
   // pipeline on a per-HSAMPLE basis, so that filters can do their Nyquist calculations
   // based on the correct equivalent output rate
   //
...
void AILCALL AIL_API_waveOutClose(HDIGDRIVER dig)
{
   ...
   if (!dig->released)
     DIG_waveOutReset(dig,dig->hWaveOut); // (May fail if device was removed)
