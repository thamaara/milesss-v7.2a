...
#if defined(IS_WIN32) || defined(IS_MAC) || defined(IS_DOS)

//############################################################################
//#                                                                          #
//# Bankfile access routines                                                 #
//#                                                                          #
//############################################################################
...
//
// Note that these routines work with platform-specific PLATFILE descriptors, 
// rather than file handles
//
// TODO: Need a BNK_is_bad_handle() function of some type...
//

#ifdef ON_MAC_USE_FSS
   static SINTa BNK_open_read_only(const char *filename)
      { 
      MSS_FILE file; 
      file.file_type=0; 
      file.file=filename;  
...
#define BANK_assert(expr)                         if (!(expr)) MSSBreakPoint()
#define BANK_malloc(bytes)                        AIL_mem_alloc_lock(bytes)
#define BANK_free(ptr)                            AIL_mem_free_lock(ptr)
#define BANK_stricmp(t1,t2)                       AIL_stricmp(t1,t2)
#define BANK_strnicmp(t1,t2,n)                    AIL_strnicmp(t1,t2,n)
#define BANK_open_read_only(filename)             BNK_open_read_only(filename)
#define BANK_seek(file_descriptor, offset)        MSS_seek(file_descriptor, offset, AIL_FILE_SEEK_BEGIN)
#define BANK_read(file_descriptor, dest, n_bytes) MSS_read(file_descriptor, dest, n_bytes)
#define BANK_close(file_descriptor)               MSS_close(file_descriptor)

#include "bankfile.inl"

// ----------------------------------------------
// Bankfile management globals
// ----------------------------------------------

SOUNDLIB *first_lib = NULL; // TODO: DPMI lock; shut down automatically 

// ----------------------------------------------
// Destroy in-memory bankfile representation
// ----------------------------------------------

DXDEF void AILEXPORT AIL_close_library(HSOUNDLIB L)
{
   //
   // Remove library from global list
   //
   ...
   //
   // Destroy bankfile handle (the actual file is already closed)
   //
...
// ----------------------------------------------
// Open bank file containing sound library
// ----------------------------------------------

DXDEF HSOUNDLIB AILEXPORT AIL_open_library(const C8 *filename,             // TODO: async version of this
                                           S32       embedded_at_offset)
{
   HBANKFILE B = NULL;

   BANKRESULT result = BankOpenReadOnly(&B,
                                         filename,  
                                         embedded_at_offset);

   if (result != BANK_OK)
      {
      AIL_set_error(BankResultText(result));
      return NULL;
      }

   //
   // Bankfile was opened successfully, and the directory
   // has been read -- we could close the file handle now, if
   // desired
   //

   HSOUNDLIB L = (HSOUNDLIB) AIL_mem_alloc_lock(sizeof(SOUNDLIB));
...
// ----------------------------------------------
// Compose a subfilename descriptor for the
// specified bankfile entry
// ----------------------------------------------
                                                 ...
                                                 S32           filename_buffer_bytes)
{
   ...
   BANKRESULT result = BankInfo(lib->B, &info);
   ...
   BANKENTRYINFO entry_info;

   result = BankEntryInfo(lib->B, resource_name, &entry_info);
   ...
   //
   // Descriptor format: *bankfile_name*len_bytes*offset_in_file[.suffix]
   //

   C8 descriptor[64];

   C8 *original_suffix = strrchr(entry_info.original_filename,'.');     // TODO: offsets >= 2G

   sprintf(descriptor,
          "%d*%d%s",
           entry_info.used_bytes,
           entry_info.offset_in_file,
           original_suffix == NULL ? "" : original_suffix);

   S32 bankfile_name_bytes = (S32) AIL_strlen(info.filename);
   S32 descriptor_bytes    = (S32) AIL_strlen(descriptor);

   //                '*'                       '*'                    '\0'
   S32 needed_bytes = 1 + bankfile_name_bytes + 1 + descriptor_bytes + 1;

   if ((filename_buffer != NULL) && (filename_buffer_bytes < needed_bytes))
      {
      AIL_set_error("Buffer too small.");
      return NULL;
      }

   if (filename_buffer == NULL)
      {
      filename_buffer = (C8 *) AIL_mem_alloc_lock(needed_bytes);
                                      ...
                                      S32 flags, AILLENGTHYCB callback)
{
  ...
  void FAR* dlsdata=AIL_mem_alloc_lock( LE_SWAP32( &((CHUNK FAR*)dls)->ckSize ) + 8);
  if (dlsdata==0)
     {
    err:
     return(0);
  ...
  while (numxmis--) {
    ...
    AIL_memcpy(dlsdata,dls,LE_SWAP32( &((CHUNK FAR*)dls)->ckSize ) + 8);
  ...
  if (dlssize)
    *dlssize=LE_SWAP32( &((CHUNK FAR*)dlsdata)->ckSize ) + 8;
