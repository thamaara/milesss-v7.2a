...
//############################################################################
//#                                                                          #
//# Return # of buffers synthesizer has generated ahead of output PCM stream #
//#                                                                          #
//############################################################################

static S32 DLS_ring_margin(void)
{
   S32 margin = DLS->build_queue_head - DLS->build_queue_tail;

   if (margin < 0)
      {
      margin = DLS->build_queue_head + (N_BUILD_BUFFERS - DLS->build_queue_tail);
      }
...
static void DLS_VOICE_note_on(VOICE FAR *V, MIDI_MESSAGE FAR *EV, S32 key, REGION FAR *RGN)
{
   ...
   V->static_atten =
      (20.0F * ((F32)(AIL_log10(16129.0F / ((F32)(EV->data_2 * EV->data_2))))));
   ...
   if (EG1_scale == ABSOLUTE_ZERO)
      ...
      V->EG1_lin_scale = (F32) AIL_pow(10.0F, V->EG1_scale / 20.0F);
      ...
      if (EG1_attack != ABSOLUTE_ZERO)
         ...
         F32 usec = (F32)(1000000.0F * AIL_pow(2.0F, F32(EG1_attack) /   (F32)(65536*1200)));

         V->EG1_attack_time      = ((S32)(usec / DLS->service_rate_uS));
         V->EG1_attack_intervals = V->EG1_attack_time;
      ...
      if (EG1_decay != ABSOLUTE_ZERO)
         ...
         F32 usec = ((F32)(1000000.0F * AIL_pow(2.0F, ((F32)(EG1_decay)) / ((F32)(65536*1200)))));
      ...
      if (EG1_release != ABSOLUTE_ZERO)
         {
         F32 usec = ((F32)(1000000.0F * AIL_pow(2.0F, ((F32)(EG1_release)) / ((F32)(65536*1200)))));
   ...
   if (EG2_scale == ABSOLUTE_ZERO)
      ...
      if (EG2_attack != ABSOLUTE_ZERO)
         ...
         F32 usec = ((F32)(1000000.0F * AIL_pow(2.0F, ((F32)(EG2_attack)) / ((F32)(65536*1200)))));
      ...
      if (EG2_decay != ABSOLUTE_ZERO)
         ...
         F32 usec = ((F32)(1000000.0F * AIL_pow(2.0F, ((F32)(EG2_decay)) / ((F32)(65536*1200)))));
      ...
      if (EG2_release != ABSOLUTE_ZERO)
         {
         F32 usec = F32(1000000.0F * AIL_pow(2.0F, F32(EG2_release) / F32(65536*1200)));
   ...
   if (LFO_delay == ABSOLUTE_ZERO)
      ...
      {
      F32 usec = F32(1000000.0F * AIL_pow(2.0F, F32(LFO_delay) / F32(65536*1200)));
   ...
   F32 LFO_frequency =
      ((F32)(AIL_pow(2.0F, (((((F32)(RGN->connection->lScale[LFO_FREQUENCY])) /
...
static void DLS_event_note_on(MIDI_MESSAGE FAR *EV)
{
   ...
   if (vn == DLS->n_voices)
      ...
      //
      // First try to find a voice that's already in its release phase...
      //
      ...
      for (ch=MAX_CHAN; (ch >= end_search) && (!found); ch--)
         {
         if (ch == PERCUSS_CHAN)
            {
            continue;
         ...
         for (vn=0; vn < DLS->n_voices; vn++)
            ...
            if ((V->trigger.channel == ch) && (V->EG1_phase == RELEASE_PHASE))
               {
               //
               // Steal this voice
               //
      ...
      if (!found)
         {
         //
         // No released voices eligible for reuse -- search for ANY
         // reclaimable voice
         //
      ...
      //
      // Give up if no voices could be reclaimed
      //
...
static void AILLIBCALLBACK DLS_serve(UINTa user)
{
   // -------------------------------------------------------
   // Determine the amount of sound data to generate for this
   // interval
   // -------------------------------------------------------
   ...
   if (DLS->last_interval_us_count == (U32)(S32)-1)
      {
   ...
   S32 elapsed_ms = (elapsed_us + 500) / 1000;

   S32 samples_this_period = ((S32)((DLS->output_sample_rate * elapsed_ms) / 1000.0F));

   //
   // Apply +/- 5% bias to sample count if ring buffers are in danger of
   // overrun or starvation
   //
   // This check is made only after the stream has been running long enough
   // to fill the entire set of ring buffers
   //

   if (DLS->buffers_filled > N_BUILD_BUFFERS)
      {
      S32 margin = DLS_ring_margin();

      if (margin <= BUILD_LOW_WATER)
         {
         //
         // Synth is running slower than its output stream -- start generating
         // more samples per interval
         //

         DLS->time_bias = 1;
         }
      else if (margin >= BUILD_HIGH_WATER)
         {
         //
         // Synth is overrunning its output stream -- start generating fewer
         // samples per interval
         //

         DLS->time_bias = -1;
         }
      else if (margin == BUILD_CENTER)
         {
         //
         // Synth is at equilibrium point in ring buffers -- generate
         // calculated number of samples per interval
         //
   ...
   samples_this_period += ((samples_this_period / 20) * DLS->time_bias);

   S32 bytes_this_period = (samples_this_period * DLS->channels_per_sample * sizeof(S32));

   bytes_this_period = min(bytes_this_period, INTERVAL_BUFFER_SIZE);

   //
   // Flush mixer interval buffer
   //

   DLS->mixer_flush(DLS->interval_buffer,
                    bytes_this_period
#ifdef IS_X86
                    ,DLS->use_MMX
#endif
   ...
   // -------------------------------------------------------
   // Merge data from all active voices into 32-bit signed interval buffer
   // -------------------------------------------------------
   ...
   for (S32 vn=0; vn < DLS->n_voices; vn++)
         ...
         switch (V->EG1_phase)
            ...
            case ATTACK_PHASE:
               ...
               if (V->EG1_attack_intervals > 0)
                  {
                  F32 percent = 1.0F - (((F32)V->EG1_attack_intervals) / ((F32) V->EG1_attack_time));

                  F32 result = V->EG1_lin_scale * percent;

                  V->EG1_atten = 95.9F;

                  if (result > 0.0F)
                     {
                     V->EG1_atten = -20.0F * (F32) AIL_log10(result / 65536.0F);
                     }
      ...
      if (V->LFO_holdoff > 0)
         ...
         F32 LFO_output = F32(AIL_sin(V->LFO_phase_accumulator));
      ...
      F32 relative_pitch = (F32) AIL_pow(2.0F, (accum_pitch / 1200.0F));
      ...
      F32 v_atten = CC7  ? (20.0F * F32(AIL_log10(16129.0F / F32(CC7  * CC7))))  : 999.0F;
      F32 e_atten = CC11 ? (20.0F * F32(AIL_log10(16129.0F / F32(CC11 * CC11)))) : 999.0F;
      ...
      if (V->mixer_operation & M_DEST_STEREO)
         {
         F32 p_left  = (CC10 != 127) ? (20.0F * F32(AIL_log10(AIL_fsqrt(127.0F / F32(127-CC10))))) : 999.0F;
         F32 p_right = (CC10)        ? (20.0F * F32(AIL_log10(AIL_fsqrt(127.0F / F32(    CC10))))) : 999.0F;
         ...
         if (left_atten > 0.0F)
            {
            left_scale = ((S32)(2048.0F / AIL_pow(10.0F, left_atten  / 20.0F)));
            }
         else
            {
            left_scale = 2048;
         ...
         if (right_atten > 0.0F)
            {
            right_scale = ((S32)(2048.0F / AIL_pow(10.0F, right_atten / 20.0F)));
            }
         else
            {
            right_scale = 2048;
...
*/
         ...
         if (mono_atten > 0.0F)
            {
            left_scale = ((S32)(2048.0F / AIL_pow(10.0F, mono_atten / 20.0F)));
            }
         else
            {
            left_scale = 2048;
...
*/
      ...
      // don't turn on resampling or filtering if we aren't rescaling...
      if ( ( playback_ratio == 65536 ) && ( V->mixer_operation & ( M_RESAMPLE | M_FILTER ) ) )
      {
        V->mixer_operation &= ~( M_RESAMPLE | M_FILTER );
      }

      //
      // Mix source wave data into destination interval buffer until source
      // data exhausted
      //

      S32  FAR *dest     = DLS->interval_buffer;
      void FAR *dest_end = AIL_ptr_add(DLS->interval_buffer, bytes_this_period);
      ...
      for(;;)
        ...
        #define NO_ZERO_VOLUME_MIX
        #ifdef NO_ZERO_VOLUME_MIX
        ...
        else
        {
          ...
          U32 dest_sample_shift = ( V->mixer_operation & M_DEST_STEREO ) ? 3 : 2;
          U32 src_sample_shift = ( V->mixer_operation & M_SRC_16 ) ? 1 : 0;

          S32 needed_dest = (S32) AIL_ptr_dif( dest_end, dest );
          S32 have_src = (S32) AIL_ptr_dif( src_end, src );
          S32 needed_src = mult64addsubandshift( needed_dest >> dest_sample_shift, playback_ratio,
                                                 0xffff, mult64anddiv(65536, 65536-(V->src_fract & 0xffff), playback_ratio), 16
                                                ) << src_sample_shift;

          if ( needed_src >= have_src )
          {
            src = src_end;

            S32 dest_from_src = shift64addsubanddiv( have_src >> src_sample_shift, 16,
                                                     65536 - ( V->src_fract & 0xffff ), playback_ratio >> 1,
                                                     playback_ratio ) << dest_sample_shift;

            dest = (S32*) AIL_ptr_add( dest, dest_from_src );
          }
          else
          {
            dest = (S32*) dest_end;

            src = AIL_ptr_add( src, needed_src );
          }
         ...
         DLS->mixer_merge(HIWORD(src),
                          ...
                          src_end_offset,
                          dest_end_offset,
                          sizeof(S16),      // DLS voices are always 16-bit mono PCM
                          &V->left_val,
                          &V->right_val,
                          playback_ratio,
         ...
         V->play_cursor = (S32) AIL_ptr_dif(src, V->wave->data);

         //
         // If end of loop reached with room left in the interval buffer,
         ...
         if ((loop_end != NULL) && (AIL_ptr_ge(src,loop_end) || AIL_ptr_lt(dest,dest_end)))
            ...
            //
            // If room left in interval buffer, keep mixing
            //
   ...
   // -------------------------------------------------------
   // Copy interval buffer contents to output ring buffer
   //
   // If none available (i.e., output buffers stalled for some reason, or data
   // being written by synthesizer at faster rate than DMA playback), dump
   // a buffer to make room
   // -------------------------------------------------------

   S32  src_remaining = bytes_this_period;
   U8  *src           = (U8 *) DLS->interval_buffer;

   while (src_remaining)
      {
      BUILD_BUFFER FAR *buffer = &DLS->build[DLS->build_queue_head];

      if (buffer->status == BUILD_FULL)
         {
         //
         // No buffers available -- discard the contents of this one
         //
      ...
      void *dest = AIL_ptr_add(buffer->block,
                               buffer->status);

      S32 dest_remaining = DLS->bytes_per_buffer - buffer->status;

      S32 len = min(src_remaining, dest_remaining);
      ...
      //
      // Subtract # of bytes copied from total amount available
      //
      ...
      if (buffer->status >= DLS->bytes_per_buffer)
         ...
         //
         // Advance output FIFO head pointer, if we're not already at
         // the tail
         //

         S32 next = (DLS->build_queue_head + 1) % N_BUILD_BUFFERS;

         if (next != DLS->build_queue_tail)
            {
            DLS->build_queue_head = next;
            }
   ...
   // buffer to output stream until the output buffer ring is half full
   // (taking into account the initial fill of both output buffers)
   // -------------------------------------------------------

   if (DLS->buffers_filled < 3) //(N_BUILD_BUFFERS/2)+2)
      {
   ...
   // -------------------------------------------------------
   // If queue contains at least one full entry, copy it to
   ...
   while (DLS->build_queue_tail != DLS->build_queue_head)
      {
      //
      // If a full build buffer is available, look for a free stream
      // buffer to receive it
      //
      // We also accept an empty buffer here, to replay in case of
      // starvation
      //
      // Partially-full buffers are rejected until complete
      //

      BUILD_BUFFER FAR *buffer = &DLS->build[DLS->build_queue_tail];
      ...
      if (!DLS->stream_poll_CB(DLS->user))
         {
         //
         // No output buffer available yet, exit
         //
      ...
      S32  FAR *src  = buffer->block;
      void FAR *dest = DLS->stream_lock_CB(DLS->user, DLS->stream_buff);

      //
      // Copy build buffer to output stream
      //
      // We use a separate routine on PS2, since we can't submit a block-interleaved
      // buffer to the MSS mixer...
      //

#ifdef IS_PS2

      PS2_noninterleaved_mixer_copy(src,
      ...
      DLS->stream_unlock_CB(DLS->user, DLS->stream_buff);
      ...
      S32 next = (DLS->build_queue_tail + 1) % N_BUILD_BUFFERS;

      if (next != DLS->build_queue_head)
         {
         DLS->build_queue_tail = next;
         }
      }
...
S32 DLS_init (SINTa FAR *lpdwHandle, //)
              S32        dwFlags,
              ...
              AILDLSUCB  stream_unlock_CB)   // Buffer unlock handler
{
   ...
   DLS->n_voices = (S32) AIL_get_preference(DLS_VOICE_LIMIT);
   ...
   AIL_set_timer_user(DLS->timer_handle, (UINTa) DLS);
...
//      ((S32)(DLS->service_rate_uS)));
   ...
   //
   // 32-bit-per-channel interval buffer is conservatively allocated at about
   // 10X the interval size at the fastest supported rate
   //

   DLS->interval_buffer = (S32 *) AIL_mem_alloc_lock(INTERVAL_BUFFER_SIZE);

   if (DLS->interval_buffer == NULL)
      {
      DLS_shutdown(*lpdwHandle, 0);
      return DLS_OUT_OF_MEM;
      }
...
//   debug_printf("WAILDLS1: Stream size=%ld\n", DLS->stream_buffer_size);
   ...
   DLS->last_interval_us_count = (U32)(S32)-1;
   DLS->time_bias = 0;
...
S32 DLS_shutdown (SINTa dwDLSHandle, //)
                  S32   dwFlags)
{
   dwFlags=dwFlags;
   ...
   //
   // Release build and interval buffers
   //
   ...
   if (DLS->interval_buffer != NULL)
      {
      AIL_mem_free_lock(DLS->interval_buffer);
      DLS->interval_buffer = NULL;
      }
