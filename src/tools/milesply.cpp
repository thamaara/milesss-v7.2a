...
#define MAX_WF_CHANNELS     256

#define N_WF_SCALES       16
static S32 WF_denom [N_WF_SCALES] = { 512, 256, 128, 64, 48, 32, 24, 16, 14, 12, 10, 8, 6, 4, 2, 1 };

static C8 *channel_names[18] = 
{ "FL", "FR", "C", "LFE", "BL", "BR", "FLoC", "FRoC", "BC", "SL", "SR", "TC", "TFL", "TFC", "TFR", "TBL", "TBC", "TBR" };
...
static POINT point_array [MAX_WF_CHANNELS][MAX_WF_RECORD_WIDTH];
...
static S32 bits=16;
static S32 chans=MSS_MC_USE_SYSTEM_CONFIG;
                                    ...
                                    FONT_EFFECTS effects = 0)
{
   ...
   TranslateCharsetInfo((DWORD *) curSysCp, &csi, TCI_SRCCODEPAGE);
...
DWORD WINAPI decompress_thread( LPVOID stuff )
{
   ...
   static S16 PCM_buffer[32768];

   S32 granularity = waveform_channels * sizeof(S16);
   S32 PCM_request_size = (sizeof(PCM_buffer) / granularity) * granularity;
...
error:
            ...
            *dest[i]++ = PCM_buffer[j+i];
            }
         }
      }

   //
   // We now have exact figures for the average source data rate and # of samples
   // in the file...
   //
                                     ...
                                     C8    *file_suffix)
{
   ...
   // precisely.
   //
   // n_waveform_samples and bytes_per_sample are estimates, which in some 
   // cases (e.g., Ogg files) may be wildly off-base.  They will be refined in the 
   // background decompression thread as the file is unpacked
   //
   ...
   n_waveform_samples = expected_bytes_per_channel / sizeof(S16);

   bytes_per_sample = (F32) (F64(source_bytes) / F64(n_waveform_samples));

   for (i=0; i < waveform_channels; i++)
      {
      oneasi.dest[i] = waveform_buffer[i];
      }
...
static S32 build_PCM_waveform_buffer(HAUDIO audio)
{
   ...
   source_addr        =  (U8 *) S->buf[0].start;
   source_bytes       =         S->buf[0].len;
   output_sample_rate =         S->playback_rate;
   waveform_channels  =         S->n_channels;
   ...
   if (S->format & DIG_F_ADPCM_MASK)
      ...
      S32 PCM_channels = PCM->n_channels;

      S32 waveform_bytes_per_channel = PCM->buf[0].len / PCM_channels;

      for (S32 i=0; i < PCM_channels; i++)
         {
         waveform_buffer[i] = (S16 *) AIL_mem_alloc_lock(waveform_bytes_per_channel);
         dest[i] = waveform_buffer[i];
         }

      S16 *buffer = (S16 *) PCM->buf[0].start;

      S32 n_samples = PCM->buf[0].len / sizeof(S16);
      ...
      {
      //
      // 16-bit PCM (including multichannel)
      //
                              ...
                              S32 wf_y0)
{
   ...
   U32 mask = 0;
   AIL_sample_channel_count(S, &mask);

   SetTextColor(wf_hDC, RGB(0,255,255));

   base_y = DIB_y0 + (max_height / 2);

   if ((waveform_channels == 1) && (mask == ~0U))
      {
      sprintf(text,"M");
      TextOut(wf_hDC, 0, base_y-4, text, (S32) strlen(text));
      }
   else
      {
      S32 index = 0;
      ...
      for (ch=0; ch < waveform_channels; ch++)
         ...
         sprintf(text,"%s",channel_names[index]);
         TextOut(wf_hDC, 0, base_y-4, text, (S32) strlen(text));
...
tryagain:
     ...
     //
      ...
      if (waveform_view_valid)
       ...
       EnableWindow( GetDlgItem(hwnd,cbMono), (audioType==AIL_QUICK_XMIDI_TYPE)?0:1 );
...
void show_help()
{
   ...
   "\tMono - this lets you force the output mode to monaural.\n"
   "\tStereo sound is slower for the software synthesizer to build.\n\n"
   ...
   switch (message)
      ...
      case WM_COMMAND:
        ...
        switch (LOWORD(wParam))
        {
          ...
          case cb16bit:
            bits=(Button_GetCheck(GetDlgItem(hwnd, cb16bit))) ? 16 : 8;
            open_file(1);
            break;

          case cbMono:
            chans=(Button_GetCheck(GetDlgItem(hwnd, cbMono))) ? 1 : MSS_MC_USE_SYSTEM_CONFIG;
            open_file(1);
            break;

          case cbFilter:
            filtered = Button_GetCheck(GetDlgItem(hwnd, cbFilter)); 
            AIL_set_preference(DLS_ENABLE_FILTERING,filtered);
            AIL_set_preference(DIG_ENABLE_RESAMPLE_FILTER,filtered);
            break;

          case cbReverb:
            reverb = Button_GetCheck(GetDlgItem(hwnd, cbReverb)); 
            set_reverb();
            break;
                   ...
                   LPSTR lpszCmdLine, int nCmdShow)
{
   ...
   if (chans!=MSS_MC_USE_SYSTEM_CONFIG)
     SendMessage(GetDlgItem(hwnd,cbMono),BM_SETCHECK,1,0);
