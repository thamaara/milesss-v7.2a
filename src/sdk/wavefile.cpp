...
void AILSFILE_start( void )
{
  if ( !locked )
  {
    ...
    AIL_vmm_lock(output_speaker_index, sizeof(output_speaker_index));
    AIL_vmm_lock(output_speaker_order, sizeof(output_speaker_order));
    AIL_vmm_lock(speaker_masks, sizeof(speaker_masks));
...
static void release_channel_buffers(HSAMPLE S)
{

   if (S->lp          != NULL) { AIL_mem_free_lock(S->lp);           S->lp          = NULL; }
   if (S->left_val    != NULL) { AIL_mem_free_lock(S->left_val);     S->left_val    = NULL; }
   if (S->right_val   != NULL) { AIL_mem_free_lock(S->right_val);    S->right_val   = NULL; }
   if (S->last_decomp != NULL) { AIL_mem_free_lock(S->last_decomp);  S->last_decomp = NULL; }
#if 0
   if (S->source_chan_to_speaker_enum != NULL) { AIL_mem_free_lock(S->source_chan_to_speaker_enum); S->source_chan_to_speaker_enum = NULL; }
#endif
   ...
   for (S32 i=0; i < m; i++)
      {
      if (S->user_channel_levels[i] != NULL) { AIL_mem_free_lock(S->user_channel_levels[i]); S->user_channel_levels[i] = NULL; }
      if (S->cur_scale[i] != NULL)           { AIL_mem_free_lock(S->cur_scale          [i]); S->cur_scale          [i] = NULL; }
      if (S->prev_scale[i] != NULL)          { AIL_mem_free_lock(S->prev_scale         [i]); S->prev_scale         [i] = NULL; }
      if (S->ramps_left[i] != NULL)          { AIL_mem_free_lock(S->ramps_left         [i]); S->ramps_left         [i] = NULL; }
      }
}
...
static S32 alloc_channel_buffers(HSAMPLE S)
{
   ...
   S->lp = (LOWPASS_INFO *) AIL_mem_alloc_lock(n * sizeof(S->lp[0]));
   if (S->lp == NULL)
   {
    fail:
   ...
   AIL_memset(S->lp, 0, n * sizeof(S->lp[0]));

   S->left_val = (S32 *) AIL_mem_alloc_lock(n * sizeof(S->left_val[0]));
   if (S->left_val == NULL) goto fail;
   AIL_memset(S->left_val, 0, n * sizeof(S->left_val[0]));

   S->right_val = (S32 *) AIL_mem_alloc_lock(n * sizeof(S->right_val[0]));
   if (S->right_val == NULL) goto fail;
   AIL_memset(S->right_val, 0, n * sizeof(S->right_val[0]));

   S->last_decomp = (S32 *) AIL_mem_alloc_lock(n * sizeof(S->last_decomp[0]));
   if (S->last_decomp == NULL) goto fail;
   AIL_memset(S->last_decomp, 0, n * sizeof(S->last_decomp[0]));
   ...
   m += (m & 1);     // (build-buffer channels are paired; this ensures we can access both without checking)
...
#if 0
   S->source_chan_to_speaker_enum = (MSS_SPEAKER *) AIL_mem_alloc_lock(n * sizeof(S->source_chan_to_speaker_enum[0]));
   if (S->source_chan_to_speaker_enum == NULL) goto fail;
   AIL_memset(S->source_chan_to_speaker_enum, 0xff, n * sizeof(S->source_chan_to_speaker_enum[0])); 
#endif
   ...
   for (i=0; (i < 32) && (src_chan < n) && (speaker_num < MAX_SPEAKERS); i++)
      ...
      if (S->channel_mask & bit_mask)
         {
#if 0
         S->source_chan_to_speaker_enum[src_chan   ] = (MSS_SPEAKER) speaker_num;   // TODO: Can be removed if nobody needs this
#endif
   ...
   for (i=0; i < m; i++)
      {
      S->user_channel_levels[i] = (F32 *) AIL_mem_alloc_lock(n * sizeof(F32));
      if (S->user_channel_levels[i] == NULL) goto fail;
      AIL_memset(S->user_channel_levels[i], 0, n * sizeof(F32));

      S->cur_scale[i]  = (S32 *) AIL_mem_alloc_lock(n * sizeof(S32));
      if (S->cur_scale[i] == NULL) goto fail;
      AIL_memset(S->cur_scale[i], 0, n * sizeof(S32));

      S->prev_scale[i] = (S32 *) AIL_mem_alloc_lock(n * sizeof(S32));
      if (S->prev_scale[i] == NULL) goto fail;
      AIL_memset(S->prev_scale[i], 0, n * sizeof(S32));

      S->ramps_left[i] = (S32 *) AIL_mem_alloc_lock(n * sizeof(S32));
      if (S->ramps_left[i] == NULL) goto fail;
      AIL_memset(S->ramps_left[i], 0, n * sizeof(S32));
      }

   if (S->format & DIG_F_STEREO_MASK)
      {
      S->user_channel_levels[0][0] = 1.0F;      // Special case: L+R stereo source channel ("0") sent to 
      S->user_channel_levels[1][0] = 1.0F;      // stereo build-buffer 0/1
      }
   else if (!(S->format & DIG_F_MULTICHANNEL_MASK))
      {
      for (i=0; i < m; i++)
         {
         S->user_channel_levels[i][0] = 1.0F;      // Mono source channel sent to all speakers by default
         }                                         // (so 3D and 5.1 panning will work without any further calls) 
      }
   else
      {
      //
      // Assign multiple source channels to output speakers
      //

      if (S->driver->logical_channels_per_sample == 1)
         {
         //
         // Mono output: all non-LFE source channels sent to FL
         //
         ...
         {
         //
         // Map all source channels in >= 2.0 HSAMPLE...
         //
      ...
      //
      // Simple normalization of source channel levels at each dest speaker 
      // to discourage clipping
      //
      // Default channel level of 2 / n_speakers chosen empirically to keep the
      // perceived volume level constant when switching the Miles Player into 'Mono'
      // mode
      // 

      for (d=0; d < S->driver->logical_channels_per_sample; d++)
         {
         S32 dest_chan = speaker_enum_to_driver_chan[d];
         ...
         if (n > 1)
            ...
            for (S32 src_chan=0; src_chan < S->n_channels; src_chan++)
               ...
               if (S->user_channel_levels[dest_chan][src_chan] > 0.0F)
                  {
                  S->user_channel_levels[dest_chan][src_chan] = norm;
                  }
               }
            }
         }
...
//##                                                                        ##
//## Available flags (ignored with some formats/platforms):                 ##
//##                                                                        ##
//##   DIG_PCM_SIGN                                                         ##
...
S32 AILCALL SS_set_sample_type( HSAMPLE S, S32 format, U32 flags, U32 channel_mask )
{
  if ( S == NULL )
  {
    return 0;
   ...
   while (!voice_block)
      ...
      switch (*(U8 FAR *) b)
         ...
         case 1:
            ...
            if (!SS_set_sample_type(sample,DIG_F_MONO_8,0,~0U))
               {
               return 0;
               }
         ...
         case 8:
            ...
            if (((BLK_8 FAR *) b)->voice_mode)
               {
               if (!SS_set_sample_type(sample,DIG_F_STEREO_8,0,~0U)) return 0;
               ...
               {
               if (!SS_set_sample_type(sample,DIG_F_MONO_8,0,~0U)) return 0;
         ...
         case 9:
            ...
            if ((((BLK_9 FAR *) b)->channels == 1) &&
               ...
               {
               if (!SS_set_sample_type(sample,DIG_F_MONO_16,DIG_PCM_SIGN,~0U)) return 0;
               }
            else if ((((BLK_9 FAR *) b)->channels == 2) &&
                     (((BLK_9 FAR *) b)->format   == 4))
               {
               if (!SS_set_sample_type(sample,DIG_F_STEREO_16,DIG_PCM_SIGN,~0U)) return 0;
               }
...
S32 AILCALL AIL_API_WAV_info(void const FAR* data, AILSOUNDINFO FAR* info)
{
   ...
   if (info->format == WAVE_FORMAT_EXTENSIBLE)
      ...
      info->channel_mask = wfex->dwChannelMask;

      info->format = WAVE_FORMAT_PCM;  // treat it just like a normal PCM .WAV, but with extra channels
      info->samples=(info->bits==0)?0:((info->data_len*8)/info->bits);  // TODO document this
...
S32 AIL_process_WAV_image( AILSOUNDINFO FAR const * info, HSAMPLE sample )
{
  S32 format=0;
  S32 sign=0;
  ...
  if (!SS_set_sample_type( sample, format, sign, info->channel_mask )) return 0;
                                            ...
                                            HSATTRIBS D)
{
   ...
   //
   // Read user channel levels, one row at a time
   //

   S8 spkr,j;
   MSS_SPEAKER src_chans[MAX_SPEAKERS],dest_chans[MAX_SPEAKERS];

   for (spkr=0; spkr < MAX_SPEAKERS; spkr++)
      {
      src_chans[spkr] = (MSS_SPEAKER) spkr;
      }

   for (spkr=0; spkr < MAX_SPEAKERS; spkr++)
      {
      for (j=0; j < MAX_SPEAKERS; j++)
         {
         dest_chans[j] = (MSS_SPEAKER) spkr;
         }

      AIL_sample_channel_levels(S,
                                src_chans,
                                dest_chans,
                                D->channel_levels[spkr],
                                MAX_SPEAKERS);
      }
   ...
   if (1)
#else
   if (S->driver->logical_channels_per_sample >= 3)
#endif
      {
                                           ...
                                           HSATTRIBS D)
{
   ...
   //
   // Set user channel levels, one row at a time
   //
   ...
   for (S32 n=0; n < D->n_filter_properties; n++)
      ...
      if ((cur != provider) && (stage != SP_OUTPUT))
         ...
         if (AIL_set_sample_processor(S,  // TODO: assumes the channel mask is still intact!
                                      stage,
                                      provider) == (HPROVIDER) -1)
            {
            return (U32) -1;
            }
         }
...
S32 AILCALL AIL_API_set_sample_info(HSAMPLE S, AILSOUNDINFO const FAR * info)
{
  ...
  if (!AIL_init_sample( S, format, sign ))
     {
     return 0;
     }
...
S32 AILCALL AIL_API_set_sample_file(HSAMPLE S, void const FAR *file_image, S32 block)
{
   ...
   if (!AIL_init_sample(S, DIG_F_MONO_16, 0))
      {
      return 0;
      }
...
S32 AILCALL AIL_API_init_sample( HSAMPLE S, S32 format, U32 flags )
{
  if ( S == NULL )
  {
    return 0;
  ...
  if (!SS_set_sample_type(S, format, flags, ~0U)) return 0;
...
void AILCALL AIL_API_sample_output_levels (HSAMPLE                S, //)
                                           MSS_SPEAKER FAR const *source_speaker_indexes,
                                           MSS_SPEAKER FAR const *dest_speaker_indexes,
                                           F32 FAR               *levels,
                                           S32                    n_levels)
...
S32 AILCALL AIL_API_sample_channel_count( HSAMPLE S, U32 FAR *mask )
{
  if ( S == NULL )
  {
    return( 0 );
    ...
    else
    {
      ...
      if (cut != S->calculated_cut)
         ...
         if ( cut < 0.0001f )
            ...
            // calculate coefficients
            S->lp[i].c.B0 =  (S32)( 2.0F * ( 1.0F - C * C ) * A );
            S->lp[i].c.B1 =  (S32)( ( 1.0F - ( AIL_fsqrt( 2.0F ) * C ) + C * C ) * A );
            }
         ...
         if ( S->lp[i].on == 0 )
            ...
            {
            S->lp[i].u.XL0 = (S32) ( ( (F32) S->lp[i].u.YL0 ) * A );
            S->lp[i].u.XL1 = (S32) ( ( (F32) S->lp[i].u.YL1 ) * A );
            S->lp[i].u.XR0 = (S32) ( ( (F32) S->lp[i].u.YR0 ) * A );
            S->lp[i].u.XR1 = (S32) ( ( (F32) S->lp[i].u.YR1 ) * A );
            }
         }

#endif     
      S->lp[i].cutoff = cutoff;
      S->lp[i].on = 1;
    }
    }
...
startsound:
      ...
      #endif
      {
      // old way: don't start until *both* buffers loaded (last_buffer > 0)
      // new way: don't start until at least *one* buffer loaded

        if ((S->service_type == 2) && ( S->tail == S->head ) && S->starved)
          return;                                                                
                                            ...
                                            S32     n_buffers)
{
   ...
   S->buf = (SMPBUF *) AIL_mem_alloc_lock(S->n_buffers * sizeof(S->buf[0]));
...
S32 AILCALL AIL_API_load_sample_buffer(HSAMPLE S, S32 buff_num, void const FAR *buffer, U32 len)
{
                    ...
                    S32                      MC_source_channel)
{
  ...
  for ( i = 0 ; i < n_dest_buffers ; i++ )
  {
    ...
    if ( ( ( left != oleft ) || ( right != oright ) ) && ( S->ramps_left[i][MC_source_channel] == 0 ) )
    {
      ...
      S->ramps_left[i][MC_source_channel]=(S32) AIL_preference[DIG_LEVEL_RAMP_SAMPLES];
    }
    ...
    if ( r > S->ramps_left[i][MC_source_channel] )
    {
      r = S->ramps_left[i][MC_source_channel];
    }
    ...
    if ( src_stereo )
    {
      ...
      if( dest_buffers[ i ].chans > 1 ) 
      {
        ...
        {
          S32 ladj = ( ( left - oleft ) << 16 ) / S->ramps_left[i][MC_source_channel];
          S32 radj = ( ( right - oright ) << 16 ) / S->ramps_left[i][MC_source_channel];
    ...
    if (S->ramps_left[i][MC_source_channel])
      ...
      left  = lv >> 16;                    
      right = rv >> 16;                    
      S->cur_scale[ i * 2     ][MC_source_channel] = left;    
      S->cur_scale[ i * 2 + 1 ][MC_source_channel] = right;   
      }
              ...
              S32      dest_buffer_size)
{
   ...
   // Set mixer operation code
   //
   // Multichannel sources are handled with multiple mono-source passes
   //
   ...
   if (dest_mono_sample_buffer == NULL)
      {
      if (dig->hw_format & (DIG_F_STEREO_MASK | DIG_F_MULTICHANNEL_MASK)) // stereo or MC output; we will use at least one 2-channel build buffer
        {
           if (S->format & DIG_F_STEREO_MASK)                             // stereo source
           {
           ...
           else                           // mono *or* multichannel source to stereo build buffer(s)
           {
             dest_buffer_size >>= 1;      // doubling mono source to stereo dest; pretend the dest buffer is half as large
             out_pos >>= 1;              
             out_pos_shift = 1;           // (i.e., the actual output position is 2x what we think it is)
           }
        }
      }

   if (S->format & DIG_F_MULTICHANNEL_MASK)
      {
   ...
   //
   // # of bytes read from source data (or requested from decoder) must be a 
   // multiple of the decoded PCM sample size.  This isn't the same as the 
   // mixer input sample size (src_sample_shift) if multiple mono passes 
   // are being made!
   //
 
   S32 bytes_per_sample = S->n_channels * sizeof(S16);
 
   S32 multipass_factor = bytes_per_sample >> src_sample_shift;
 
   if (multipass_factor << src_sample_shift != bytes_per_sample)
      {
      MSSBreakPoint();
      }
 
   needed_src *= multipass_factor;
   ...
   U8 MC_buffer_start      [ DIG_PROCESS_BUFFER_SIZE + 16 + 15 ];  // deinterlacing target buffer
 
   U8 FAR * process_buffer = (U8 FAR*) ( ( ( (UINTa) process_buffer_start ) + 15 ) & ~15 );
   U8 FAR * MC_buffer      = (U8 FAR*) ( ( ( (UINTa) MC_buffer_start )      + 15 ) & ~15 );
 
   S32 process_buffer_bytes = (DIG_PROCESS_BUFFER_SIZE / bytes_per_sample) * bytes_per_sample;
   ...
   if (S->pipeline[SP_ASI_DECODER].active)
      ...
      if ( S->last_decomp[0] != 65536 )
         ...
         for (S32 i=0; i < S->n_channels; i++)
            {
            ( ( S16 FAR* ) process_buffer)[ i ] = (S16) S->last_decomp[i];
            }
 
         src_adjust = bytes_per_sample;
         process_ptr += src_adjust;
         }
...
uncomp_advance:
...
#ifdef MSS_FLT_SUPPORTED
   if ((S->n_active_filters > 0) || (S->lp[0].on))
      {
      filters_enabled = 1;
      }
#endif
   ...
   //
   // If this is a multichannel source sample, make one mono pass per source channel,
   ...
   void const FAR *MC_src         = src;
   S32             MC_needed_src  = needed_src;
   S32             MC_needed_dest = needed_dest;
   S32             MC_src_fract   = S->src_fract;
...
do_MC_src_channel:
  ...
  if (S->format & DIG_F_MULTICHANNEL_MASK)
     ...
     S16 *out = (S16 *) MC_buffer;

     S32 n_samples = MC_needed_src / bytes_per_sample;
   ...
   if (dest_mono_sample_buffer == NULL)
      ...
      for (S32 b = 0; b < dig->n_build_buffers; b++)   
         ...
         F32 right_scale = SC [ LVL [index+1] ] * dig->D3D.speaker_dry_reverb_response[index+1];
         ...
         if (B->chans == 1)
            {
            S->cur_scale[index]  [MC_chan] = (S32) ((2048.0F * left_scale * S->user_channel_levels[index][MC_chan] * S->auto_3D_channel_levels[index]) + 0.5F);
            S->cur_scale[index+1][MC_chan] = 0;
            
            if (S->cur_scale[index][MC_chan] >= 2048) S->cur_scale[index][MC_chan] = 2048;

            if (S->cur_scale[index][MC_chan] == 0)
               {
            ...
            {
            S->cur_scale[index]  [MC_chan] = (S32) ((2048.0F * left_scale  * S->user_channel_levels[index  ][MC_chan] * S->auto_3D_channel_levels[index  ]) + 0.5F);
            S->cur_scale[index+1][MC_chan] = (S32) ((2048.0F * right_scale * S->user_channel_levels[index+1][MC_chan] * S->auto_3D_channel_levels[index+1]) + 0.5F);
   ...
   if (zero_volume 
       && 
      ((!using_MC) || (MC_chan < S->n_channels-1)))
      {
      // --------------------------------------------------------------------------------
      ...
      S->left_val [MC_chan] = 0;
      S->right_val[MC_chan] = 0;
      ...
      if ((S->src_fract == 0x20000000) && (op & M_FILTER))
         ...
         if (S->format & DIG_F_16BITS_MASK)
            ...
            if ( S->format & DIG_F_STEREO_MASK )
               ...
               if (S->flags & DIG_PCM_ORDER)
                  {
                  S->left_val [0]  = (S16) LE_SWAP16(&( (S16 FAR*) src )[ 1 ]);
                  S->right_val[0]  = (S16) LE_SWAP16(&( (S16 FAR*) src )[ 0 ]);
                  }
               else
                  {
                  S->left_val [0]  = (S16) LE_SWAP16(&( (S16 FAR*) src )[ 0 ]);
                  S->right_val[0]  = (S16) LE_SWAP16(&( (S16 FAR*) src )[ 1 ]);
                  }
               }
            else
               {
               if (S->flags & DIG_F_MULTICHANNEL_MASK)
                  {
                  S->left_val [MC_chan] = 
                  S->right_val[MC_chan] = (S16) LE_SWAP16(&( (S16 FAR*) src )[ MC_chan ]);
                  }
               else
                  {
                  S->left_val [0] = (S16) LE_SWAP16(&( (S16 FAR*) src )[ 0 ]);
                  S->right_val[0] = S->left_val[0];
                  }
               }
            }
         else
            ...
            if ( S->format & DIG_F_STEREO_MASK )
               ...
               if (S->flags & DIG_PCM_ORDER)
                  {
                  S->left_val [0] = ( (U8 FAR*) src )[ 1 ] - 128;
                  S->right_val[0] = ( (U8 FAR*) src )[ 0 ] - 128;
                  }
               else
                  {
                  S->left_val [0] = ( (U8 FAR*) src )[ 0 ] - 128;
                  S->right_val[0] = ( (U8 FAR*) src )[ 1 ] - 128;
                  }
               }
            else
               {
   ...
   if (!using_process)
      ...
      if (!using_MC)
         {
         S->buf[ S->tail ].pos += (S32) AIL_ptr_dif(src, mixer_input);
         }
      }
   else
      {
      ...
      if (src >= src_end)
         ...
         if (using_MC)
            {
            S->last_decomp[MC_chan] = *(S16 FAR *) src;
            }
         else
            {
            for (S32 i=0; i < S->n_channels; i++)
               {
               S->last_decomp[i] = ( (S16 FAR*) src )[ i ]; 
               }
            }
         }
      }
   ...
   if (using_MC)
      ...
      if (!using_process)
         {
         S->buf[S->tail].pos += (needed_src * S->n_channels);
         }
      }
      ...
      if (S->buf[S->tail].reset_ASI)
      {
        ...
        if ( S->pipeline[SP_ASI_DECODER].active )
        {
          ...
          if (ASI->STREAM_SEEK_POS != (UINTa) -1)
            ...
            ASI->ASI_stream_property(ASI->stream,
                                     ASI->STREAM_SEEK_POS,
                                     0,&S->buf[ S->tail ].reset_seek_pos, &S->buf[ S->tail ].reset_seek_pos);
            }
...
void AILSFILE_end(void)
{
   if (locked)
      ...
      AIL_vmm_unlock(output_speaker_index, sizeof(output_speaker_index));
      AIL_vmm_unlock(output_speaker_order, sizeof(output_speaker_order));
      AIL_vmm_unlock(speaker_masks, sizeof(speaker_masks));
