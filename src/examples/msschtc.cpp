...
/* @cdep pre 
  ...
  $requiresbinary(comctl32.lib comdlg32.lib wsock32.lib)
*/
...
C8          *stream_buffer[N_STREAM_BUFFERS];
S32          stream_buffer_num;
...
static void update_graph(void const *data, S32 len)
{
  S32 i;
  F64 a=0;
...
S32 CALLBACK RECV_stream_CB(UINTa     user, //)
                            void FAR *dest,
                            S32       bytes_requested,
                            S32       offset)
{
...
S32 CALLBACK XMIT_stream_CB(UINTa     user, //)
                            void FAR *dest,
                            S32       bytes_requested,
                            S32       offset)
{
...
DWORD WINAPI receive_ASI_thread_procedure(LPVOID pParam)
{
...
DWORD WINAPI transmit_ASI_thread_procedure(LPVOID pParam)
{
   //
                   ...
                   LPSTR lpszCmdLine, int nCmdShow)
{
   ...
   if (input_source == SOURCE_FILE)
      ...
      RIB_INTERFACE_ENTRY ASISTR[] =
         {
         { RIB_FUNCTION, "ASI_stream_open",           (UINTa) &XMIT_stream_open,           RIB_NONE },
         { RIB_FUNCTION, "ASI_stream_close",          (UINTa) &XMIT_stream_close,          RIB_NONE },
         { RIB_FUNCTION, "ASI_stream_process",        (UINTa) &XMIT_stream_process,        RIB_NONE },
         };
      ...
      transmit_thread = CreateThread(NULL,
                                     0,
                                     transmit_ASI_thread_procedure,
                                     NULL,
                                     0,
   ...
   RIB_INTERFACE_ENTRY RECVSTR[] =
      {
      { RIB_FUNCTION,   "ASI_stream_property",       (UINTa) &RECV.ASI_stream_property,       RIB_NONE },
      { RIB_FUNCTION,   "ASI_stream_open",           (UINTa) &RECV.ASI_stream_open,           RIB_NONE },
      { RIB_FUNCTION,   "ASI_stream_seek",           (UINTa) &RECV.ASI_stream_seek,           RIB_NONE },
      { RIB_FUNCTION,   "ASI_stream_close",          (UINTa) &RECV.ASI_stream_close,          RIB_NONE },
      { RIB_FUNCTION,   "ASI_stream_process",        (UINTa) &RECV.ASI_stream_process,        RIB_NONE },
      { RIB_PROPERTY,   "Output sample rate",        (UINTa) &RECV.OUTPUT_SAMPLE_RATE,        RIB_NONE },
      { RIB_PROPERTY,   "Output sample width",       (UINTa) &RECV.OUTPUT_BITS,               RIB_NONE },
      { RIB_PROPERTY,   "Output channels",           (UINTa) &RECV.OUTPUT_CHANNELS,           RIB_NONE },
      { RIB_PROPERTY,   "Requested sample rate",     (UINTa) &RECV.REQUESTED_RATE,            RIB_NONE },
      };
   ...
   receive_thread = CreateThread(NULL,
                                 0,
                                 receive_ASI_thread_procedure,
                                 NULL,
                                 0,
   ...
   while (active)
      ...
      if ((input_source == SOURCE_FILE) && (file_active))
         ...
         if (current_send_offset == TRANSMIT_PACKET_SIZE)
            ...
            while (fill < TRANSMIT_PACKET_SIZE)
               {
               S32 result = (S32) fread(&send_buffer[fill], 1, TRANSMIT_PACKET_SIZE - fill, src_file);
      ...
      if (!AIL_sample_buffer_available(stream))
         {
         continue;
         }
...
//         printf("WARNING: Wanted data but none ready yet!\n");
         ...
         memset(stream_buffer[stream_buffer_num],
                0,
                receive_buffer_size);
         }
      else
         ...
         memcpy(stream_buffer[stream_buffer_num],
      ...
      //
      // Submit stream buffer to MSS and enable playback
      //

      AIL_load_sample_buffer(stream,
                             MSS_BUFFER_HEAD,
                             stream_buffer[stream_buffer_num],
                             receive_buffer_size);

      stream_buffer_num = (stream_buffer_num + 1) % N_STREAM_BUFFERS;

      AIL_start_sample(stream);
      }
