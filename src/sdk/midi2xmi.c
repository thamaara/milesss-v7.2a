...
#if defined( IS_BE ) || defined( IS_32 )

#define read32(ptr) BEU_SWAP32(((U32*)ptr))
#define read16(ptr) BEU_SWAP16(((U16*)ptr))
...
static U32 MIDI_construct(MIDI FAR*MIDI)
{
   ...
   MIDI->tick_time = ((U32) (50000000)) / (U32) MIDI->division;
            ...
            case EV_META:
               ...
               switch (type)
                  ...
                  case META_TEMPO:
                     ...
                     MIDI->tick_time = (100 * MIDI_get_24(MIDI,trk)) /
                        (U32) MIDI->division;
...
static U32 XMIDI_construct(XMIDI FAR*XMIDI)
{
   ...
   XMIDI->q_int = (100000000 / (U32) XMIDI->AILH.quantization);
   XMIDI->DDA_sum = 0L;
   XMIDI->interval = 0L;
   XMIDI->delta = 0L;
...
{
#ifdef IS_PS2
   XMIDI->DDA_sum += (F32) (S32) MIDI->event_time *
                     (F32) (S32) MIDI->tick_time;

   while (XMIDI->DDA_sum >= (F32) XMIDI->q_int)
      {
      XMIDI->DDA_sum -= (F32) XMIDI->q_int;
      XMIDI->interval++;
      XMIDI->delta++;
      }
#else
...
static U32 XMIDI_log_branch(XMIDI FAR*XMIDI, MIDI FAR*MIDI)
{
   ...
   XMIDI->RBRN.brn[b].offset = (U32) AIL_ptr_dif(XMIDI->EVNT.ptr, XMIDI->EVNT.base);
   ...
   for (i=0;i<MAX_NOTES;i++)
      ...
      if ((XMIDI->note_chan[i] == channel) && (XMIDI->note_num[i] == note))
         ...
         if (offset)
            ...
            len = (U32) AIL_ptr_dif( XMIDI->EVNT.ptr, XMIDI->note_next[i]);
            ...
            for (j=0;j<MAX_NOTES;j++)
               ...
               if (XMIDI->note_chan[j] != 255)
                  {
                  if (((SINTa) XMIDI->note_next[j]) - ((SINTa) src) >= 0L)
                     {
...
static U32 XMIDI_IFF_write_blocks(XMIDI FAR*XMIDI, HMEMDUMP out)
{
...
#ifdef IS_BE
  {
    ...
    {
      MEM_LE_SWAP16( (void FAR*)&XMIDI->RBRN.brn[i].bnum );
      MEM_LE_SWAP16( (void FAR*)&XMIDI->RBRN.brn[i].offset );
    }
...
static void XMIDI_compile(char const FAR*src, U32 src_len, HMEMDUMP out, U32 quant)
{
   ...
   midi=(MIDI FAR*)AIL_mem_alloc_lock(sizeof(*midi));
   if (midi==0)
     abend(1,2);

   xmidi=(XMIDI FAR*)AIL_mem_alloc_lock(sizeof(*xmidi));
   if (xmidi==0) {
...
#if defined(__BORLANDC__) || defined(IS_MAC) || defined(IS_PS2) || defined(IS_PS3) || defined(IS_WII)
   if (setjmp(errhand)) {
#else
   if (_setjmp(errhand)) {
#endif
...
{
   U32      catlen_off;
   volatile HMEMDUMP tmp=0;
   volatile HMEMDUMP XMID=0;
   ...
   XMIDI_compile((char const FAR *)MIDI, MIDI_size, tmp, (S32) AIL_get_preference(MDI_SERVICE_RATE));
