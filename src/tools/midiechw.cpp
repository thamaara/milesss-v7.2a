...
/* @cdep pre
  ...
  $requires(midiechw.rc)
  $requiresbinary(winmm.lib dsound.lib comdlg32.lib)
*/
...
UINTa   MIDI_in;   // -1 if invalid/unused
UINTa   MIDI_out;  // -1 if invalid/unused
UINTa   WAVE_out;  // -1 if invalid/unused
                         ...
                         LPVOID  lpContext)
{
   ...
   SINTa i = SendDlgItemMessage(hCombo,
                              IDC_DSOUND,
                              CB_ADDSTRING,
                              0,
                     (LPARAM) lpszDesc);
...
void DLG_init(void)
{
   ...
   for (i=0; i < n; i++)
      ...
      SINTa index = SendDlgItemMessage(hDlg,
                                     IDC_MIDIIN,
                                     CB_ADDSTRING,
                                     0,
                                     (LPARAM) in.szPname);
...
void AILCALLBACK BAR_timer(UINTa junk)
{
   U32 i;
...
S32 CALLBACK DLS_poll_callback(UINTa user)
{
   return AIL_sample_buffer_available((HSAMPLE) user);
}
...
void FAR * CALLBACK DLS_lock_callback(UINTa user, S32 buffer_section)
{
...
void CALLBACK DLS_unlock_callback(UINTa user, S32 buffer_section)
{
                    ...
                    LPSTR     lpCmdLine, int       nCmdShow )
{
   ...
   if ((DAT.WO_status) && (WAVE_out != -1))
      ...
      AIL_set_preference(DIG_PREFERRED_WO_DEVICE, WAVE_out);
      }
   else if ((DAT.DS_status) && ((SINTa) DS_out != -1))
      {
      AIL_set_preference(DIG_USE_WAVEOUT, 0);
      AIL_set_preference(DIG_PREFERRED_DS_DEVICE, (DWORD_PTR) DS_out);
      }
   ...
   if (dig == NULL)
      {
      alert_box("Error","AIL_open_digital_driver() failed: %s\n",AIL_last_error());
      return 0;
      }
   ...
   if ((strlen(DAT.DLSDLL_fn))          && 
      ...
      if (hDLSLibrary == NULL)
         ...
         {
         *(UINTa *) &lpfnAIL_DLSOpen      = (UINTa) GetProcAddress(hDLSLibrary,"DLSMSSOpen");
         *(UINTa *) &lpfnDLSOpen          = (UINTa) GetProcAddress(hDLSLibrary,"DLSOpen");
         *(UINTa *) &lpfnDLSClose         = (UINTa) GetProcAddress(hDLSLibrary,"DLSClose");
         *(UINTa *) &lpfnDLSLoadFile      = (UINTa) GetProcAddress(hDLSLibrary,"DLSLoadFile");
         *(UINTa *) &lpfnDLSUnloadFile    = (UINTa) GetProcAddress(hDLSLibrary,"DLSUnloadFile");
         *(UINTa *) &lpfnDLSUnloadAll     = (UINTa) GetProcAddress(hDLSLibrary,"DLSUnloadAll");
         *(UINTa *) &lpfnDLSGetInfo       = (UINTa) GetProcAddress(hDLSLibrary,"DLSGetInfo");
         *(UINTa *) &lpfnDLSCompactMemory = (UINTa) GetProcAddress(hDLSLibrary,"DLSCompactMemory");
         *(UINTa *) &lpfnDLSSetAttribute  = (UINTa) GetProcAddress(hDLSLibrary,"DLSSetAttribute");
